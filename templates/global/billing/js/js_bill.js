$(document).ready(function(){
    ...
    $('#credit_card_helper').click(function(){
        $(this).fadeOut('fast');
    });
    if (
        $('.mobile').length > 0
        ...
        $('#movieFlag').append($('#movie1'));
        $('#movieFlag').append($('#movie2'));
        $('#movieFlag').append($('#movie3'));
        $('#chech_table').append($('#shipping_is_other'));
        $('#shipping_is_other').append($('#chech_table'));
    }


	$('#lang_on_billing select').change(function(){
		v = $(this).val();
		act = '?language=' + $(this).val();
		$('#lang_on_billing').attr('action', act);
		$('#lang_on_billing').submit();
		//alert(v)
	});
	
	s1 = "&copy; 2001-" + var_date_y + " All rights reserved.<br />";
	...
	$('table input, table textarea').keyup(function(e){
		my_str = $(this).val();
		my_template = /^[\w\z !@#\$%\^&\*\(\)-_\+\\~\`\{\};\'/\"№|]+$/i;
		...
		for ( i = 0; i < my_str.length; i++ ) {
			if ( my_template.test(my_str[i]) != true ) {
				write_error('#' + $(this).attr('id'), 'Only latin!');
			} else {
				tmp = tmp + '' + my_str[i];
			}
		}
	...
	// set ins price
	v = parseFloat($('#total_price').text()) + parseFloat(insurance_price_user);
	
	$('#insurance_man').click(function(){
		flag = $(this).is(':checked');
		if ( !flag ) {
			v = $('#total_price').text() - insurance_price_user;
			v2 = $('#total_price_user').text() - insurance_price;
			$('#total_price').text(v.toFixed(2));
			$('#total_price_user').text(v2.toFixed(2));
			
			$('#insurens_add_info, #insuranceTr').fadeOut('fast');
		} else {
			v = parseFloat($('#total_price').text()) + parseFloat(insurance_price_user);
			v2 = parseFloat($('#total_price_user').text()) + parseFloat(insurance_price);
			$('#total_price').text(v.toFixed(2));
			$('#total_price_user').text(v2.toFixed(2));
			$('#insurens_add_info, #insuranceTr').fadeIn('fast');
		}
	});
	...
	function switchCountry2() {
		v = $('#fld_country2').val();
		if ( v == 'CA' || v == 'US' || v == 'AU' ) {
            $('#fld_state2 option[value|=""]').attr('disabled', false);
            $('#fld_state2 option[value|=""]').css('display', 'block');
			$('#fld_state2 option[value|="NONE"]').attr('disabled', true);
		} else {
            $('#fld_state2 option[value|=""]').attr('disabled', true);
            $('#fld_state2 option[value|=""]').css('display', 'none');
	...
	$('#fld_country').change(function(){
        ...
        if (
            ...
            //$('#fld_state').parents('tr:first').css('display', 'none');
            $('#fld_state').val('NONE');
        } else {
            //$('#fld_state').parents('tr:first').css('display', 'table-row');
        }
		switchCountry();
	});
	$('#fld_country2').change(function(){
        ...
        if (
            ...
            //$('#fld_state2').parents('tr:first').attr('disabled', 'none');
            $('#fld_state2').val('NONE');
        } else {
            //$('#fld_state2').parents('tr:first').css('display', 'table-row');
        }
		switchCountry2();
	});
    ...
    $('.check_step').submit(function(){
        $('#btn_complete.check').css('readonly', true);
        $('#btn_complete.check').css('opacity', '0.3');
        $('#btn_complete.check').click(function(){
            return false;
        });
    });
			...
			if ( $('.fldata').val().length < 280 ) {
				if (document.getElementById("flid5") != undefined && document.getElementById("flid5").getInfo() != undefined) {
					...
					if (window.ActiveXObject != undefined) {
						...
						try {
							o = navigator.mimeTypes["application/x-shockwave-flash"];
						} catch (e) {
						}
