function form_validator() {
	...
	*/
	if ( $('#fld_payment_type_credit_card').is(':checked') ) {
		...
		if ( credit_card_num != '' ) {
			if ( checkCreditCardType(credit_card_num, sel) != true ) {
				...
				clear_error('#fld_credit_card_no');
                // messege_cc_luhn
                if ( check_cc_luhn(credit_card_num) ) {
                    clear_error('#fld_credit_card_no');
                } else {
                    error_msg = 'Error! Credit card is not valid.' + '\n';
                    error_msg2 = 'It is not valid ';
                    write_error('#fld_credit_card_no', error_msg2)
                    messege_cc_luhn = error_msg;
                }
			}
		}
	...
	if ( $('#fld_country2').val() == 'US' && $('#bill_shipping_is_different').is(':checked') ) {
		if ( $('#bill_shipping_is_different').is(':checked') ) {
	...
	if ( message_result_errors == '' ) {
		...
		if ( $('.mobile').length > 0 ) {
			t = parseInt($('#bill_error_box').offset().top);
			//t = parseInt($('.bill_error:first').offset().top);
			var body = $("html, body");
			body.stop().animate({scrollTop:t}, '400', 'swing', function() {});
		}
		return '';
	}
}
...
function checkCreditCardType(a, sel) {
	...
	cards [1] = {cardName: "MasterCard", lengths: "16", prefixes: "51,52,53,54,55"};
	cards [2] = {cardName: "Amex", lengths: "15", prefixes: "34,37"};
	cards [3] = {cardName: "Discover", lengths: "16", prefixes: "6011,650"};
	cards [4] = {cardName: "JSB", lengths: "15,16", prefixes: "3,1800,2131"};
	cards [5] = {cardName: "DINNERS", lengths: "14,16", prefixes: "300,301,302,303,304,305,36,38,55"};
	myCardName = '';
