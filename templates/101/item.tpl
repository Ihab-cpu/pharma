        ...
        <table class="product_descr_tbl">
            <tr>
                ...
                <td>
                    ...
                    <div class="av">{"Availability:"|mytranslate} <span>{"In Stock"|mytranslate} ({$availability} packages)</span></div>
                    <!--div class="ex">{"Exp. Date: Approx."|mytranslate} {$approx_date} </div-->
                    <div class="clear"></div>
        ...
        {if $resultArr.synonims|@count > 0}
	        ...
	        {if $resultArr.synonims|@count > 0}
        		...
        		<span class="oneLineHeight">
        			<span class="viewAll"><i></i>{"View all"|mytranslate}</span>
	        		<span class="nowHeight">
			            <div>{"Other names of "|mytranslate} {$resultArr.name}:&nbsp;</div>
            ...
            {foreach item=v key=k from=$resultArr.packings}
                ...
                <table class="dosage_table">
                    ...
                    <tr>
                        ...
                        <td class="b saving">{if $v2.save != ''}{$currency_symbol}{$v2.save|string_format:"%.2f"}{/if}</td>
                        <td>{if $v2.save != ''}<span class="color1 old-price">{$currency_symbol}{$v2.save+$v2.pack_price|string_format:"%.2f"}</span> {/if}<b>{$currency_symbol}{$v2.pack_price|string_format:"%.2f"}</b></td>
                        <td style="width:130px;"><a class="add_to_cart" href="{$BASE_FOLDER}categories/{$url_arr.folders.1}/{$config_array.pill_prefix}{$url_arr.folders.2}{$config_array.pill_postfix}/?buy={$k2}">{"ADD TO CART"|mytranslate}</a></td>
                    </tr>
                    {/foreach}
                </table>
