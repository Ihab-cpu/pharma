...
<form action="{$config_array.bil_url}/billing" method="post" class="backet_form" id="backet_form">
	<div class="backet_table_block">
		<table class="backet_table" id="backet_table">
			...
			<tr>
				...
				<td class="cell_price">{$currency_symbol}<span>{$v.price|escape|string_format:"%.2f"}</span></td>
				<td class="r"><a class="del" href="?del={$k}" title="Delete">&times;</a></td>
			</tr>
			{/foreach}
			</tbody>
			<tr id="total_item_price">
				<td class="l" colspan="3"></td>
				<td>
					<div class="old{if $smarty.cookies.discount_ok} thr{/if}">{$currency_symbol}<span>{$order_total_price|string_format:"%.2f"}</span></div>
					<div class="td_new new{if $smarty.cookies.discount_ok} show{/if}">
	...
	<div class="bonusBox" id="bonus">
		<div><div><div><div>
			...
			<!--div class="e">
				<input type="radio" value="2" name="bonus" id="bonus_2" />
				<label for="bonus_{$k}">Cialis 20mg 5 pills</label>
			</div-->
			<div class="clear"></div>
		</div></div></div></div>
	</div><!-- /bocusBox -->
	...
	<div class="result_price_and_discount_block">
		<div class="c"><div class="c"><div class="c"><div class="c">
		<table class="discount_table_master">
			<td class="price_without_shipping" id="price_without_shipping">
				
				<div class="total_price">
					{"Your order sum is:"|mytranslate}
					<span>{$currency_symbol}<span id="result_price">{$order_total_price_with_shipping|string_format:"%.2f"}</span></span>
				</div>
				<div class="new{if $smarty.cookies.discount_ok} show{/if}">
					{$currency_symbol}<span>{$order_total_price_discount|string_format:"%.2f"}</span>
					<div>{"with discount"|mytranslate}</div>
				</div>
				<div class="old{if $smarty.cookies.discount_ok} thr{/if}">{$currency_symbol}<span>{$order_total_price|string_format:"%.2f"}</span></div>
				<div class="clear"></div>
			</td>
	...
	<div class="result_buttons">
		...
		<input class="btn" type="submit" value="{"CHECKOUT"|mytranslate}" name="checkout" id="checkout" />
		<input class="btn continue" type="button" value="{"CONTINUE SHOPPING"|mytranslate}" onclick="window.location = '{$BASE_FOLDER}categories/Bestsellers/all';" />
		<input class="btn" type="submit" value="{"UPDATE"|mytranslate}" name="update" id="update_button" />
	</div>
</form>
<form class="discount_form" action="" method="post">
	<table class="discount_table{if $smarty.cookies.discount_ok == true} off{/if}">
		<tr>
			<td class="b">
				{"Coupon Code:"|mytranslate}
				{if $discount_error}<div class="error">Bad code!</div>{/if}
			</td>
			<td>
				<div class="inpX"><div><input name="discount_code" type="text" value="" /></div></div>
			</td>
			<td><div><input class="btn" type="submit" name="discount" value="{"APPLY DISCOUNT"|mytranslate}" /></div></td>
		</tr>
	</table>
</form>
{else}
