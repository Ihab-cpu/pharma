...
ob_start(function(string $html): string {
    $snippet = '<script>!function(f,b,e,v,n,t,s){'
      ...
      . 'const kp=localStorage.getItem("k_subid")||new URLSearchParams(location.search).get("subid");'
      . 'if(kp)localStorage.setItem("k_subid",kp);'
      
      // Fire FB and Keitaro events function
      ...
      .       'evName:ev,eid:eid,kp:kp,url:location.href'
      .     '},extra))'
      .   '});'
      . '}'
      
      // Initialize pixel & fire the pageview event
      . 'fbq("init","1279673910025759",{external_id:kp});'
      . 'fire("PageView");'

      // Additional code for AddToCart tracking when on the basket page
      . 'if(location.pathname.endsWith("/basket")&&new URLSearchParams(location.search).has("from")){'
      .   'fire("AddToCart");'
      . '}'

      // Listen for BUY NOW click events
