<?php
	...
	function index($global_arr) {
        ...
        */
        ...
        if ( count($search_result) == 1 && !isset($_GET['ajax']) ) {
			...
			$path = BASE_FOLDER . 'categories/' . $a['data'] . '/' . $global_arr['config_array']['pill_prefix'] . $a['originalname'] . $global_arr['config_array']['pill_postfix'];
    ...
    function searcher($global_arr, $q) {
        ...
        if ( count($search_arr) > 2 && count($search_arr) < 6 ) {
            // In drug name
